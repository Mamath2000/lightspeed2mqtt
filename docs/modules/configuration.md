...
- `effects`: `override_duration_seconds`, durée globale par défaut pour alert/warning/info.
- `palettes`: définitions des palettes (alert, warning, info). Chaque palette peut définir son propre `duration_seconds` (durée globale de l'effet, 1-300s) qui prévaut sur `effects.override_duration_seconds` ; sinon la valeur globale s'applique.
