...
def test_override_completion_in_auto_mode_fully_releases_sdk(tmp_path, fake_lighting):
    """En mode auto, la fin d'un effet doit faire un release() complet (LogiLedShutdown) :
    LogiLedInit() coupe tout effet Logitech natif en cours, donc tant que la session SDK
    reste ouverte, Logitech ne reprend jamais réellement la main (au mieux on rejoue un
    snapshot figé). Le coût (~2s de réinit au prochain effet) est accepté."""
    service, controller = build_service(tmp_path, fake_lighting)
